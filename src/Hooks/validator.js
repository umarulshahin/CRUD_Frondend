...
const Signup_validator = (username,lastname,email,phone,password,con_password,navigate) => {
    ...
    if (username && lastname && email && phone && password && con_password){
        ...
        const isCon_Password=/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,20}$/.test(con_password)
        ...
        if (!isPassword) return "Password must be at least 8 characters long, contain at least one uppercase letter, one lowercase letter, one digit, and one special character."
        if (!isCon_Password) return "Password must be at least 8 characters long, contain at least one uppercase letter, one lowercase letter, one digit, and one special character."
        if (password != con_password) return "Passwords do not match."
            ...
            try{
                const response = await axios.post(signup_URLS,{data:data},{
                     headers: {
